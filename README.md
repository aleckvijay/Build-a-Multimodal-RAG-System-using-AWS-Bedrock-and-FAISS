...
Request Model Access on AWS Bedrock
Data Setup - Use the provided data to create s3 storage or directly use from local folder.
Environment Creation - Virtual environment Creation
AWS CLI - Credentials Setup
Streamlit Deployment on EC2 instance (Optional)
1. Request Model Access from AWS Bedrock
Step 1: Log in to AWS
Go to the AWS Management Console.
Sign in with your AWS account credentials.
Step 2: Navigate to Amazon Bedrock
In the AWS Management Console, use the search bar to find "Bedrock."
Select "Bedrock" from the search results. getting_started
Step 3: Initiate Access Request
On the Amazon Bedrock service page, click "Get Started."
In the left window, click "Manage Model Access." model_access
Step 4: Select the Models
In the model access section, choose "Amazon" from the list of available models.
This will display all available models, including Titan Text Embeddings V2 and Claude Sonnet Multimodal. Please ensure that your region has both of these models example US-EAST-1. You might need to change region if you don't see them in the list of available models.
Step 5: Request Model Access
Select the required models and scroll to the bottom of the page and click "Request Model Access."
After submitting the request, you will be redirected to the Bedrock overview page, where you can see the status of your access request (pending or granted).
2. Data Setup on S3
...
Install the project requirements using pip: pip install -r requirements.txt
...
For Windows: Open the Command Prompt by pressing Win + R, typing "cmd", and pressing Enter.
...
Prerequisites
AWS Account: Ensure you have an active AWS account. You can create one at AWS Sign-Up.
AWS CLI Installed: You must have AWS CLI installed on your machine. Follow the AWS CLI installation guide if it's not already installed.
...
AWS Access Key ID: Your unique access key for AWS services. AWS Secret Access Key: Your secret key for authentication. Default Region Name: The AWS region you want to use by default (e.g., us-west-2, us-east-1).
...
Deployment Steps
Refer the following project - Learn to Build an End-to-End Machine Learning Pipeline - Part 2
...
Gain superuser access by running: sudo su

Updating and Verifying Python

Update the EC2 instance with the latest packages: apt update

Verify Python installation: python3 --version

Installing Python Packages apt install python3-pip

Transferring Files to EC2 Use SCP to transfer your Streamlit application code to the EC2 instance:

scp -i "your-key.pem" -r path/to/your/app ubuntu@your-ec2-instance-public-ip:/path/to/remote/location

Setting Up Streamlit Application Change the working directory to the deployment files location:
...
Running the Streamlit Application Test your Streamlit application (Use external link): streamlit run app.py

For a permanent run, use nohup: nohup streamlit run app.py

Cleanup and Termination To terminate the nohup process:
